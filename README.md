...
- Middleware для выполнения логики до и после вызовов методов других сервисов.
...
Фреймворк поддерживает middleware для вызовов методов других сервисов. Middleware подключаются при создании `Service`, регистрируются через DI-контейнер и могут выполнять логику до или после вызова клиента сервиса.

Подробное описание API и примеры проверки прав пользователя перед вызовом метода, аудита успешного ответа и обработки ошибок находятся в документе [Middleware](./docs/middleware.md).
