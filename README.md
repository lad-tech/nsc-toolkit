...
1. Файл `service.schema.json`. Каждый сервис описывается в формате json. 
   ...
   - `kvBuckets` 一 опциональная секция JetStream Key-Value: объект «имя бакета → опции»;
     - `history` 一 глубина истории по ключу (1–64, по умолчанию 1);
     - `ttl` 一 время жизни значений в секундах;
     - `max_bytes` 一 максимальный размер бакета в байтах;
     - `max_value_size` 一 максимальный размер одного значения в байтах;
     - `storage` 一 хранилище: `file` или `memory`.

    Файл позволяет описать все возможности создаваемого сервиса:
     - синхронные методы;
     - генерируемые сообщения;
     - настройки стримов для сообщений;
     - KV-бакеты JetStream (get/put/delete/watch). 
...
1. Класс `Service`. Чтобы создать сервис, необходимо создать экземпляр этого класса и вызвать метод `start`.
   ...
   - `kvBuckets` 一 опциональный конфиг KV-бакетов из схемы; при старте бакеты создаются или привязываются, доступны в методах через декоратор `@kv('имя_бакета')`.
...
2. Класс `Client`. Чтобы создать клиент для сервиса, необходимо расширить класс `Client` и описать в нем все доступные извне вызовы методов. Для вызова метода используется приватный метод `Client.request`. При использовании схемы с секцией `kvBuckets` импортируйте и передавайте `kvBuckets` в конструктор (как `events` и `Ref`). Пример: [MathService index](./examples/MathService/index.ts), с KV — [KvService index](./examples/KvService/index.ts).
...
4. Декораторы. Применение декораторов:
   ...
   * `@kv('bucketName')`: инъекция JetStream Key-Value бакета в метод. Имя бакета должно быть объявлено в `service.schema.json` → `kvBuckets`. В handler доступны методы бакета: `get`, `put`, `delete`, `purge`, `watch`, `keys` и др. (см. [NATS KV](https://docs.nats.io/nats-concepts/jetstream/key-value-store)). Пример — [examples/KvService](./examples/KvService).
